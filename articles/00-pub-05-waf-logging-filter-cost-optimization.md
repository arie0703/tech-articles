...
などといった点が考えられますが、静的ファイルに対するALLOWログは調査対象のログではないめ、保持する必要性はそこまでありませんでした。
...
静的ファイルの URI パターンを `regex_pattern_set` に切り出し、それにマッチしたら `static-asset` ラベルを付けて **`count`** としてリクエストを通過するルールを追加します。
...
resource "aws_wafv2_regex_pattern_set" "static_assets" {
  ...
  regular_expression { regex_string = "^/apple-touch-icon[a-z0-9.-]*\\.png$" }
}
...
DROPしたいログに付与する用のラベルの他、別ラベルも利用している場合にはLogging Filterの設定に注意が必要です。
...
という条件の場合は優先的に保持したい1. に該当するラベルを保持（KEEP）するようにルール設定した上で、DROPルールを後述する必要があります。
...
`logging_filter` の `filter` は最大 3 件、`condition` は filter あたり最大 5 件です。URI を直接条件に並べていくと早晩詰まるので、regex_pattern_listに該当するリクエストパターンによって付与する**ラベルを活用する方が運用がしやすいかと思います。**
...
- `S3-Egress-Bytes-WAFLogs`  **「WAF → S3 へのログ配信」** にかかる Vended Logs 課金で、配信バイト数に比例して課金されるため、コスト最適化のためにログとして記録するものを取捨選択する必要がある。
